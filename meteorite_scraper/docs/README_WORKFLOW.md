# MeteorAI â€” Annotation & Training Workflow
...
```
Scrape (web)       â†’ push_to_label_studio.py  â”€â”
                                                â”œâ†’ auto_annotate.py â†’ Label Studio â†’ retrain
Sort media (local) â†’ import_images.py         â”€â”˜
```
...
### Step 1 â€” Classify and sort

```bash
python scripts/sort_media.py                  # sorts unsorted_media/ by default
python scripts/sort_media.py --source path/   # or specify a directory
python scripts/sort_media.py --dry-run        # preview without copying
python scripts/sort_media.py --conf 0.7       # stricter confidence (default 0.6)
```

This runs the scene classifier and **moves** files into two directories:
- `sorted_in_situ/` â€” meteorite on the ground (useful)
- `sorted_not_in_situ/` â€” studio, hand, display case, etc. (kept for reference)

Images below the confidence threshold are left in place â€” review them
...
`meteorites_in_situ/` is **not** skipped â€” those images will be classified
and routed normally, landing in `sorted_in_situ/` for import.

### Step 2 â€” Review and curate
...
### Step 3 â€” Import into the pipeline

```bash
python scripts/import_images.py                        # imports sorted_in_situ/ by default
python scripts/import_images.py --source path/to/dir   # or any other directory
python scripts/import_images.py --dry-run              # preview without writing
python scripts/import_images.py --skip-ls              # skip Label Studio push
```
...
You can import from any directory directly â€” for example if you've added
images to the classifier training set and want them in the pipeline too:

```bash
python scripts/import_images.py --source "classifier_training_data/in_situ"
```
...
images (gitignored â€” images stay on disk but aren't tracked in the repo).
...
```bash
python scripts/auto_annotate.py                  # unannotated tasks only (default)
python scripts/auto_annotate.py --all --replace  # refresh predictions on all tasks
python scripts/auto_annotate.py --conf 0.4       # raise confidence threshold (default 0.25)
python scripts/auto_annotate.py --containment 0.5 # stricter spatial filtering (default 0.3)
python scripts/auto_annotate.py --dry-run        # preview without pushing
```
...
2. Load the prediction as your starting point (lightning bolt / Predictions
   panel) â€” this copies the model's boxes into the editor
3. Fix what's wrong: delete bad boxes, resize off-target ones, add missing ones
4. Hit **Submit** â€” this saves a human annotation
...
- `auto_annotate.py --replace` is always safe â€” it never touches saved annotations

If you already have a correct human annotation on a task, ignore the
prediction â€” it's just decorative and has no effect on training.
...
python scripts/train_model.py --epochs 150 --model yolov8s.pt --clean

# Skip re-export if annotations haven't changed
python scripts/train_model.py --skip-export --epochs 150 --model yolov8s.pt --clean
```
...
| `yolov8s.pt` | ~11M | 100â€“500 images â† current default |
| `yolov8m.pt` | ~26M | 500â€“2000 images |
...
```bash
python scripts/auto_annotate.py --all --replace
```
...
| Class | Notes |
|---|---|
| `meteorite` | The whole meteorite â€” always required |
| `fusion_crust` | Dark glassy exterior â€” must be within a meteorite box |
| `regmaglypts` | Thumbprint depressions â€” must be within a meteorite box |
| `metal_flake` | Shiny metal inclusions â€” must be within a meteorite box |
| `scale_reference` | Ruler or coin for scale â€” independent |

Classes need â‰¥ 5 annotated examples to be included in training
(`MIN_CLASS_EXAMPLES` in `train_model.py`). Currently `regmaglypts` (1) and
`metal_flake` (0) are excluded â€” annotate more to unlock them.
...
- **Already-annotated tasks:** don't bother fixing the prediction â€” the human
